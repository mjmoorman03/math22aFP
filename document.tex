...
however, are only defined by one operation which you are free to choose. Vector 
spaces, however, require two operations, scalar multiplication and vector addition specifically.
But we've already shown that vector addition in vector spaces satisfies our axioms 
of groups, which means that the set of elements that composes our vector space along 
with the operation vector addition is a group! This is a very important observation,
as it means that we can use the properties of groups to prove properties of vector spaces.

\paragraph*{Definition 3:} As a bit of an aside, because we have also shown that 
vector addition in our vector space $V$ is commutative as per the axioms 
of vector spaces, this means that the order in which we perform our group operation on 
any two given vectors does not matter, so they form an \textbf{abelian group} - 
defined by the commutativity of the group operation. 
